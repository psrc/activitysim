...
if create_survey_files:
    ...
    # If a person makes a school tour, make sure they are coded as a student...
    ...
    # Borrowed from annotate_persons
    ...
    # First, look at all trip records to identify most common school trip location
    ...
    for col in ['dest_lat_x', 'dest_lat_y']:
        df_imputed['imputed_school_loc_lat'] = df.groupby('person_id')['dest_lat_x'].agg(lambda x: pd.Series.mode(x)[0])
        df_imputed['imputed_school_loc_lng'] = df.groupby('person_id')['dest_lng_x'].agg(lambda x: pd.Series.mode(x)[0])
    ...
    # Recalculate students missing school locations now
    ...
    df_imputed = pd.DataFrame()
    for col in ['dest_lat_x', 'dest_lat_y']:
        df_imputed['imputed_work_lat'] = df.groupby('person_id')['dest_lat_x'].agg(lambda x: pd.Series.mode(x)[0])
        df_imputed['imputed_work_lng'] = df.groupby('person_id')['dest_lng_x'].agg(lambda x: pd.Series.mode(x)[0])
    ...
    #person['workplace_zone_id'] = person['workplace_zone_id'].fillna(person['maz_id'])
    person.drop('maz_id', inplace=True, axis=1)

    #  If someone is coded as a worker but doesn't make a work trip or have a usual location, change their worker type
    # FIXME: this may be altering the person structure
    person.loc[person['pemploy'].isin([1,2]) & (person['workplace_zone_id'].isnull()), 'pemploy'] = 3

    # Recalculate person type only
    expr_df = pd.read_csv(r'\\modelstation2\c$\Workspace\activitysim\activitysim\examples\example_psrc\scripts\person_expr_activitysim_2.csv')
    ...
    # Write some checks to make sure 
...
if convert_survey:
    ...
    #person = pd.read_csv(os.path.join(survey_file_dir,'person.csv'))
    person = pd.read_csv(r'C:\Workspace\activitysim\activitysim\survey_data\survey_persons.csv')
    ##trip = pd.read_csv(r'R:\e2projects_two\activitysim\survey\elmer_trip_geocoded.csv')
    trip = pd.read_csv(os.path.join(survey_file_dir,'trip.csv'))
    ...
    # Filter for people from person file only
    # FIXME: we have excluded some people for being students and not having school info
    # These lead to removed trips/tours and joint tour data
    # We would like to retain this data; how can we 
